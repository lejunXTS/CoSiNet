...
class Pair(Dataset):
    ...
    def __getitem__(self, index):   #通过_sample_pair方法，用于提取两帧有效图片（有效的定义是图片目标的面积和高宽等有效约束条件）的索引
                                    #在得到这两帧图片和对应groundtruth之后通过定义好的transforms进行处理
                                    #transforms是siamfctransforms类的实例化对象，该类中主要继承了resize图片大小和各种裁剪方式等
